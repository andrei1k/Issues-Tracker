...
export class IssueService {
    async getIssues(projectId: number): Promise<Issue[]> {
            const response = await fetch(`http://${IP_ADDRESS}:3001/projects/${projectId}/issues/all/`, {
                method: 'GET',
                headers: {
    ...
    async removeIssue(projectId: number, issueId: number): Promise<void> {
        const response = await fetch(`http://${IP_ADDRESS}:3001/projects/${projectId}/issues/remove/${issueId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`
    ...
    async getIssue(issueId: number): Promise<Issue> {
        const response = await fetch(`http://${IP_ADDRESS}:3001/issues/${issueId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`
