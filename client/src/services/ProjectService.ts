...
class ProjectService {
    async viewProjects(userId: number): Promise<Project[]> {
        const response = await fetch(`http://${IP_ADDRESS}:3001/projects/view/${userId}`, {
            method: 'GET',
            headers: {
    ...
    async viewSortedProjectsByDate(userId: number, increasing: boolean) {
        ...
        const response = await fetch(
                `http://${IP_ADDRESS}:3001/projects/view/sorted-date/${wayOfSort}/${userId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}` 
    ...
    async viewSortedProjectsByName(userId: number, increasing: boolean) {
        ...
        const response = await 
            fetch(`http://${IP_ADDRESS}:3001/projects/view/sorted-name/${wayOfSort}/${userId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}` 
    ...
    async addProject(userId:number, projectName: string) {
        ...
        const response = await fetch(`http://${IP_ADDRESS}:3001/projects/add/${userId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`
