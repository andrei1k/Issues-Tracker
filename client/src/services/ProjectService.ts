...
class ProjectService {
    ...
    async viewSortedProjectsByName(userId: number, increasing: boolean) {
        ...
        const response = await 
            fetch(`http://127.0.0.1:3001/projects/view/sorted-name/${wayOfSort}/${userId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}` 
    ...
    async addProject(userId:number, projectName: string) {
        ...
        const response = await fetch(`http://127.0.0.1:3001/projects/add/${userId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`
    ...
    async getUsersFromProject(projectId: number) {
            const response = await fetch(`http://127.0.0.1:3001/projects/users`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`,
    ...
    async addUserInProject(projectId: number, firstName: string, lastName: string, email: string) {
        const response = await fetch(`http://127.0.0.1:3001/projects/add-user`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getToken()}`,
